<?php
include_once("includes/header.php");
?>
<body>
    <div class="container" style="width: 70%">
    <p><h1 style="color: #2a9d8f"> Analyses des règles</h1></p>
    </div>
    <div class = "container border border-3">
    <br/>
    <p>
    Notre objectif est de mettre en place une maquette illustrant la méthode expliquant les règles qui nous semble répondre à la problématique du mieux possible . Pour cela il nous sera nécessaire d’avoir analysé au préalable les règles du jeu Complots de façon à pouvoir les présenter de la manière la plus judicieuse possible.
    </p>
    <p>
    Pour cela, nous avons lu les règles fournies avec le jeu puis nous avons réalisé des parties. Cela a permis de mettre en évidence des incertitudes. qui ne nous étaient pas apparues lors de la simple lecture du livret. Nous prenons alors soin d’en prendre note.
    </p>
    <a href="pdf/regles.pdf" target="_blank"> Règles du jeu Complots</a>
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
</div>
<div class="container" style="width: 70%">
    <p><h1 style="color: #e9c46a"> Travail sur les règles</h1></p>
    </div>
    <div class = "container border border-3">
    <br/>
    <p>
        Suite à nos observations, nous avons mis nos prises de notes en commun et nous avons pris connaissance des différentes réponses du questionnaire. Cela nous a fait réaliser qu’il serait compliqué d’améliorer considérablement l’aspect ludique de la lecture des règles. En effet, il était évident que la clarté des règles actuelles était améliorable, en revanche, concernant le divertissement, il semblait de plus en plus difficile d’avoir un réel impact.
    </p>
    <p>
        Pourtant nous nous sommes réunis la semaine suivante pour réaliser un brainstorming sur l’application miro afin de préparer la réalisation de notre maquette, prochaine étape dans notre planning. En effet, étant un groupe de seulement 3 personnes, nous ne pouvons pas nous permettre d’être trop ambitieux et devons indéniablement laisser de côté des idées intéressantes. 
    </p>
    <p>
        Le brainstorming s'est donc déroulé de la manière suivante. Nous disposions de quatre minutes chacun afin d’écrire sur l’application les différents points qui se sont illustrés lors de nos observations et que nous pouvons retravailler. Nous avons ensuite pris quatre minutes pour déterminer individuellement des solutions permettant de résoudre les problèmes évoqués précédemment. Enfin, nous avions le droit à quatre votes afin de préciser quels problèmes nous semblaient les plus importants et quatre autres votes afin de voter pour les solutions qui nous semblaient les plus pertinentes et les plus réalisables. Cela nous à alors permis de prioriser notre travail et de le visualiser plus clairement ce à quoi notre maquette allait correspondre.
    </p>
    <table>
        <td width="50%">
            Nous avons donc obtenu le tableau miro suivant :
        </td>
        <td>
            <img src="images/tableau_miro.png" width=400>
        </td>
    </table>
    <p class="my-3">
    Il nous est ensuite possible de classer chaque solution. Pour cela, nous leur attribuons des valeurs selon le nombre de votes qu’elles ont reçues
    </p>
</div>
</body>

<?php
include_once("includes/footer.php");
?>